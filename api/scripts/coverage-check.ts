...
// Words stripped when computing the core token for a company name.
// Includes: English prepositions/articles, industry generic terms, legal suffixes.
const NAME_STOPWORDS = new Set([
  // English articles / prepositions (too common to be distinctive identifiers)
  ...
  'financial',
  'software',
  // Role/industry generic terms — too common as standalone tokens (e.g., "security" matches
  // "Application Security @ Wiz" when searching for "Halborn Security")
  'security', 'services', 'management', 'systems', 'data',
  // Industry noise: industry-generic terms that appear in many unrelated company names
  'web3', 'blockchain', 'crypto', 'defi', 'dao',
  // Financial sector generics (e.g. "Xapo Bank" → "bank" matches Fifth Third Bank, Bank of America)
  'bank', 'exchange', 'wallet',
  // Common English adjectives used in company/title contexts
  // "prime" matches "Prime Trading Desk @ Kraken" when searching for "Sigma Prime"
  'prime',
]);

// Clean a single name-part down to an array of distinctive words.
function normalizePart(raw: string): string[] {
  return raw
    .toLowerCase()
    // strip common TLD suffixes before removing punctuation
...
// Returns the set of core tokens for a company name.
// Splits on " / " to handle entries like "MakerDAO / Sky".
function coreTokens(name: string): Set<string> {
  ...
  for (const part of parts) {
    ...
    // Also add the compacted form for multi-word names ("makerdao")
